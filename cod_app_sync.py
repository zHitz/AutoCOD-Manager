...
def adb_back(serial, count=1):
    for _ in range(count):
        subprocess.run(
            [ADB_PATH, "-s", serial, "shell", "input", "keyevent", "4"]
        )  # KEYCODE_BACK
        time.sleep(1.5)
...
class LDPlayerScanner:
    ...
    def scan_profile_data(self, img):
        ...
        res["Power"] = self.ocr(
            self.preprocess_roi(img[y1:y2, x1:x2]), "0123456789.,KMB"
        )
        return res
    ...
    def scan_resources_data(self, img):
        ...
        for r in ["gold", "wood", "ore", "mana"]:
            ...
            val_t = self.parse_num(
                self.ocr(self.preprocess_roi(img[y1:y2, x1:x2]), "0123456789.,KMB")
            )
...
def main():
    ...
    # Broadcast Navigate
    broadcast_tap(devices, 18, 10)
    time.sleep(5.0)
    broadcast_tap(devices, 550, 200)
    time.sleep(0.5)
    broadcast_tap(devices, 550, 200)
    time.sleep(3.0)
    ...
    for serial in devices:
        ...
        data = s.retry_scan(
            s.scan_profile_data, lambda x: x and len(x.get("Name", "")) > 2
        )
    ...
    # Broadcast Navigate
    broadcast_tap(devices, 925, 500)
    time.sleep(5.0)
    broadcast_tap(devices, 780, 500)
    time.sleep(5.0)
    broadcast_tap(devices, 75, 180)
    time.sleep(5.0)
    broadcast_tap(devices, 620, 100)
    time.sleep(5.0)
    ...
    for serial in devices:
        ...
        s = LDPlayerScanner(serial)
        data = s.retry_scan(
            s.scan_resources_data, lambda x: x and any(v > 0 for v in x)
        )
    ...
    print("\n>>> PHASE 3: HALL CHECK")
    # Broadcast Navigate
    broadcast_tap(devices, 456, 111)
    time.sleep(5.0)
    broadcast_tap(devices, 380, 116)
    time.sleep(5.0)
    ...
    # Broadcast Navigate
    broadcast_tap(devices, 639, 232)
    time.sleep(5.0)
    broadcast_tap(devices, 545, 267)
    time.sleep(5.0)
    ...
    # Broadcast Navigate
    broadcast_tap(devices, 750, 80)
    time.sleep(5.0)
    for _ in range(3):
        broadcast_swipe(devices, 100, 450, 100, 100, 500)
        time.sleep(1.0)
    broadcast_tap(devices, 100, 375)
    time.sleep(5.0)
...
def print_report(report):
    print("\n" + "=" * 120)
    print(
        f"{'SERIAL':<15} {'NAME':<15} {'POWER':<12} {'GOLD':<10} {'WOOD':<10} {'ORE':<10} {'MANA':<10} {'HALL':<5} {'MKT':<5} {'PET':<5}"
    )
    print("-" * 120)
    ...
    for serial, data in report.items():
        ...
        print(
            f"{serial:<15} {name:<15} {power:<12} {res_str[0]:<10} {res_str[1]:<10} {res_str[2]:<10} {res_str[3]:<10} {hall:<5} {mkt:<5} {pet:<5}"
        )
    print("=" * 120 + "\n")

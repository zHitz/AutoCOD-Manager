...
from workflow import adb_helper, core_actions
...
HERO_TEMPLATE_PATHS = [
    os.path.abspath(os.path.join(root_dir, "workflow", "templates", "icon_markers", name))
    for name in HERO_TEMPLATE_NAMES
]
...
def locate_template(
    ...
    if max_val < threshold:
        # print(f"[{serial}] Template not found (conf: {max_val:.3f}).")
        return None
...
def process_visible_heroes(serial: str, detector: GameStateDetector, templates: list, max_retries: int = 3) -> int:
    ...
    for attempt in range(max_retries):
        ...
        # Keep searching as long as we find ANY of the templates
        while found_any:
            found_any = False
            for template in templates:
                ...
                if match:
                    ...
                    found_any = True
                    time.sleep(POST_TAP_DELAY_SEC)
                    # Break the template loop to re-scan the viewport after a successful tap/chat
                    break
...
def chat_with_heroes_one_clockwise_round(serial: str, detector: GameStateDetector) -> int:
    print(f"\n[WORKFLOW] CHAT_WITH_HERO one clockwise round on {serial}\n")

    templates = []
    for path in HERO_TEMPLATE_PATHS:
        t = load_template(path)
        if t is not None:
            templates.append(t)
    
    if not templates:
        print(f"[{serial}] [FAILED] No templates could be loaded.")
        return 0
...
def main():
    ...
    templates_dir = os.path.abspath(os.path.join(root_dir, "workflow", "templates"))
    detector = GameStateDetector(adb_path=config.adb_path, templates_dir=templates_dir)

    total_handled = chat_with_heroes_one_clockwise_round(serial, detector)
    print(f"\n[WORKFLOW DONE] Hero chats handled in one round: {total_handled}")

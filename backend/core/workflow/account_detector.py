...
class AccountDetector:
    ...
    def check_account_name(self, serial: str, target: str, check_type: str = "text", tesseract_config: str = '--psm 6') -> tuple:
        """
        ...
        data = pytesseract.image_to_data(enhanced, config=tesseract_config, output_type=Output.DICT)
        ...
        for i in range(len(data['text'])):
            ...
            if word and target_lower in word.lower():
                ...
                if conf != '-1' and int(conf) > 40:
                    ...
                    print(f"[OCR] Found '{word}' at center ({center_x}, {center_y}) | conf: {conf}%")
                    return (target, center_x, center_y)
                    
        return None

    def check_account_name_basic(self, serial: str, target: str, check_type: str = "text", tesseract_config: str = '--psm 6') -> tuple:
        """
        ...
        # Output.DICT gives us access to coordinates and confidences
        data = pytesseract.image_to_data(gray, config=tesseract_config, output_type=Output.DICT)
        ...
        for i in range(len(data['text'])):
            ...
            if word and target_lower in word.lower():
                ...
                if conf != '-1' and int(conf) > 40:
                    ...
                    print(f"[OCR Basic] Found '{word}' at center ({center_x}, {center_y}) | conf: {conf}%")
                    return (target, center_x, center_y)
...
def main():
    ...
    print(f"[INFO] Capturing screen and scanning...")
    ...
    if result:
        ...
        if screen is not None:
            ...
            cv2.putText(debug_img, f"{target_text} ({cx}, {cy})", (cx + 20, cy - 20),
